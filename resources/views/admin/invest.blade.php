                            ...
                            <div class="col-sm-12">
                                <table class="table table-bordered table-striped table-vcenter js-dataTable-full dataTable no-footer" id="DataTables_Table_0" aria-describedby="DataTables_Table_0_info">
                                    ...
                                    <tr>
                                        ...
                                        <th class="sorting" tabindex="0" aria-controls="DataTables_Table_0" rowspan="1" colspan="1" aria-label="Name: activate to sort column ascending">Package</th>
                                        ...
                                        <th class="d-none d-sm-table-cell sorting"  tabindex="0" aria-controls="DataTables_Table_0" rowspan="1" colspan="1" aria-label="Email: activate to sort column ascending">User</th>
                                    ...
                                    @foreach($invest as $item)
                                        <tr class="odd">
                                            ...
                                            <td class="fw-semibold">{{ $item->investment_plans->name }}</td>
                                            <td class="fw-semibold">${{ $item->amount }}</td>
                                            <td class="d-none d-sm-table-cell"> {{ $item->user->name }}</td>
                                            <td class="d-none d-sm-table-cell"> {!! $item->status() !!}</td>
                                            <td>
                                                @if($item->status == 1)
                                                <a href="{{ route('admin.approve_invest', $item->id) }}" class="btn btn-sm btn-success mb-1">Approve</a>
                                                @else
                                                @endif
                                                <form method="POST" action="{!! route('admin.invest_delete', $item->id) !!}" accept-charset="UTF-8">
                                                    <input name="_method" value="DELETE" type="hidden">
                                                    {{ csrf_field() }}

                                                    <div class="btn-group btn-group-xs pull-right" role="group">
                                                        <button data-toggle="tooltip" data-placement="top" type="submit" class="btn  btn-sm btn-danger" onclick="return confirm(&quot;Delete Investment?&quot;)">
